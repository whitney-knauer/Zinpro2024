...
ggplot(aes(x = hoursfrom, y = standm, color=D0Fed2TxGrp)) +
  ...
  geom_line() +  
    ...
    values = c(
      ...
      "MCT" = "red4")) +
  labs(
    title = "Standing Time in the 3d around the second feeding",
    x = "Hours from feeding Time",
    y = "Standing Time (min)") +
    scale_y_continuous(limits = c(0, 60), breaks = seq(0, 60, by = 5))+
    scale_x_continuous(limits = c(-8, 64), breaks = seq(-8, 64, by = 8))
...
stepgrp |> 
ggplot(aes(x = hoursfrom, y = steps, color=D0Fed2TxGrp)) +
  theme_classic() +
  geom_point() +  
  geom_line() +  
    scale_color_manual(
    ...
    values = c(
      ...
      "MCT" = "red4")) +
  labs(
    title = "Steps in the 3d around the second feeding",
    x = "Hours from feeding Time",
    y = "Steps (n)") #+
    #scale_y_continuous(limits = c(0, 60), breaks = seq(0, 60, by = 5))+
    #scale_x_continuous(limits = c(-8, 64), breaks = seq(-8, 64, by = 8))
...
lbgrp <- merged_data |> 
  ...
  group_by(CalfID, hoursfrom, D0Fed2TxGrp) |>  
  summarize(
    `Lying Bouts` = sum(`Lying Bouts`, na.rm = TRUE)) |> 
  group_by(hoursfrom, D0Fed2TxGrp) |>  
  summarize(
    lb = mean(`Lying Bouts`, na.rm = TRUE)) |> 
  ungroup()


lbgrp |> 
ggplot(aes(x = hoursfrom, y = lb, color=D0Fed2TxGrp)) +
  theme_classic() +
  geom_point() +  
  geom_line() +   
  labs(
    title = "Lying Bouts in the 24hrs around the second feeding",
    x = "Hours from feeding Time",
    y = "Steps (n)")
```
...
migrp |> 
ggplot(aes(x = hoursfrom, y = mi, color=D0Fed2TxGrp)) +
  theme_classic() +
  geom_point() +  
  geom_line() +  
    scale_color_manual(
    ...
    values = c(
      ...
      "MCT" = "red4")) +
  labs(
    title = "MI in the 3d around the second feeding",
    x = "Hours from feeding Time",
    y = "MI (n)")
```
...
#RS is the best fit, but not really great
...
emm <- emmeans(mod_rs, ~ hour * D0Fed2TxGrp)
summary(emm)
pairs(emm, by = "hour")
...
mod_ar1 <- lme(
  fixed = Lying ~ hour * D0Fed2TxGrp + Replacement,
  random = ~ 1 | CalfID,
  correlation = corAR1(form = ~ TimeNum | CalfID),
  data = hourfull
)
...
mod_ar1 <- lme(
  fixed = LB ~ hour * D0Fed2TxGrp + CrossbredYN,
  random = ~ 1 | CalfID,
  correlation = corAR1(form = ~ TimeNum | CalfID),
  data = hourfull
)
...
mod_ar1 <- lme(
  fixed = Steps ~ hour * D0Fed2TxGrp + CrossbredYN,
  random = ~ 1 | CalfID,
  correlation = corAR1(form = ~ TimeNum | CalfID),
  data = hourfull
)
...
And messing around with using replacement rather than crossbred.
...
mod_ar1 <- lme(
  fixed = standm ~ tffeed * D0Fed2TxGrp + Replacement,
  random = ~ 1 | CalfID,
  correlation = corAR1(form = ~ TimeNum | CalfID),
  data = hr2
)
...
#More accessible color palette: 

ggplot(emm_df, aes(x = tffeed, y = emmean, color = D0Fed2TxGrp, group = D0Fed2TxGrp)) +
  geom_line(aes(linetype = D0Fed2TxGrp), linewidth = 0.7) + 
  geom_point(size = 1.5) +
  # Using ColorBrewer's colorblind-friendly Dark2 palette
...
mod_ar1 <- lme(
  fixed = laym ~ tffeed * D0Fed2TxGrp + Replacement,
  random = ~ 1 | CalfID,
  correlation = corAR1(form = ~ TimeNum | CalfID),
  data = hr2
)
...
ggplot(emm_df, aes(x = tffeed, y = emmean, color = D0Fed2TxGrp, group = D0Fed2TxGrp)) +
  geom_line(aes(linetype=D0Fed2TxGrp, group=D0Fed2TxGrp)) +
  geom_point() +
  scale_color_manual(
    name = NULL, 
    values = c(
...
#not sig different from each other. Going to choose AR1

mod_ar1 <- lme(
  fixed = lb ~ tffeed * D0Fed2TxGrp + Replacement,
  random = ~ 1 | CalfID,
  correlation = corAR1(form = ~ TimeNum | CalfID),
  data = hr2
)
...
mod_base <- lme(
  fixed = Steps ~ tffeed * D0Fed2TxGrp,
  random = ~ 1 | CalfID,
  data = hr2
)
...
mod_ar1 <- lme(
  fixed = Steps ~ tffeed * D0Fed2TxGrp + Replacement,
  random = ~ 1 | CalfID,
  correlation = corAR1(form = ~ TimeNum | CalfID),
  data = hr2
)
...
ggplot(emm_df, aes(x = hour, y = emmean, color = bottleyn)) +
  ...
  scale_color_manual(
    ...
    values = c(
      ...
      "N" = "red4")) +
  geom_errorbar(aes(ymin = lower.CL, ymax = upper.CL), width = 0.2) +
  labs(x = "Hour from Second Feeding", y = "Standing Time (min)", title = "Adjusted Mean Standing Time") +
  theme_classic()+
    scale_y_continuous(limits = c(0, 20), breaks = seq(0, 20, by = 5))+
    scale_x_continuous(limits = c(-8, 8), breaks = seq(-8, 8, by = 2)) +
...
ggplot(emm_df, aes(x = tffeed, y = emmean, color = bottleyn)) +
  ...
  labs(
    x = "Minutes from Second Feeding",
    y = "Standing Time (min)",
  title = "Adjusted Mean Standing Time")+
  theme_classic() +
   theme(legend.position = "right", 
        text = element_text(family = "Arial", size = 14))+  
