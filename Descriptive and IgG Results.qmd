...
clean |> 
  summarize (mean = mean(F1ColostIgGgL), 
             sd = sd(F1ColostIgGgL), 
             median = median(F1ColostIgGgL), 
             min = min(F1ColostIgGgL), 
             max = max (F1ColostIgGgL))

clean |> 
  group_by(D0Fed1Batchnum) %>% 
  summarize (mean = mean(F1ColostIgGgL), 
             sd = sd(F1ColostIgGgL))
...
MCCS |> 
  ...
  summarize (mean = mean(F2ColostIgGgL, na.rm=TRUE), 
            ...
            median =median (F2ColostIgGgL, na.rm=TRUE), 
             min = min (F2ColostIgGgL, na.rm=TRUE), 
             max = max (F2ColostIgGgL, na.rm=TRUE))
...
clean |> 
  ...
  summarize (mean = mean(F2ColostIgGgL), 
             sd = sd(F2ColostIgGgL), 
             median =median (F2ColostIgGgL), 
             min = min (F2ColostIgGgL), 
             max = max (F2ColostIgGgL))

clean |> 
  group_by (D0Fed2Batchnum) |> 
  summarize (mean = mean(F2ColostIgGgL), 
             sd = sd(F2ColostIgGgL))
...
clean |> 
  summarize (mean = mean(D1MilkRefusemL),
             ...
             min = min(D1MilkRefusemL), 
             max = max (D1MilkRefusemL),
...
lm_batch <- lm(Result8hrgL ~ D0Fed1Batchnum, data=clean)
summary(lm_batch) #no
...
lm_batch <- lm(Result24hrgL ~ D0Fed2Batchnum, data=clean)
summary(lm_batch)

#model
#tested REPLACEMENT in the model, but the model with SexMF and CrossbredYN fit the data better (lower AIC)
glmIgG24.2 <- lm(Result24hrgL ~ D0Fed2TxGrp  + Replacement + CrossbredYN, data=clean)
summary(glmIgG24)
plot(glmIgG24)
car::Anova(glmIgG24, type=3)


glmIgG24 <- lm(Result24hrgL ~ D0Fed2TxGrp  + SexMF + CrossbredYN, data=clean)
summary(glmIgG24)
plot(glmIgG24)
car::Anova(glmIgG24, type=3)
