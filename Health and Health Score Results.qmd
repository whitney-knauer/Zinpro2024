...
clean<- 
  clean |> 
  mutate(refuseMeasure = ifelse(D1MilkRefusemL >= 500, 1, 0)) 

clean <-
  clean %>% 
  mutate(refuse1L = ifelse(D1MilkRefusemL >= 1000, 1, 0))
...
#        0  1
#   CSB 79 11 = 11/90=12.2
#   CST 70 21 = 21/91=23.0
#   MCB 79 12 = 12/91=13.2
#   MCT 80 13 = 13/93=13.9
#   MRB 73 15 = 15/88=17.0
...
em_link_hs <- emmeans(model, specs = "D0Fed2TxGrp")
mrb_contrasts <- contrast(em_link_hs, method = "trt.vs.ctrl", ref = 1)
summary(mrb_contrasts, type = "response", infer = c(TRUE, TRUE))
```
...
#        0  1
#   CSB 71 16 = 16/87=18.4
#   CST 57 30 = 30/87=34.1
#   MCB 64 23 = 23/87=26.4
#   MCT 64 26 = 26/90=28.8
...
#  CSB 85  3 = 3/88=3.4
#  CST 81  8 = 8/89=8.9
#  MCB 75 13 = 13/88=14.7
#  MCT 77 13 = 13/90=14.4
#  MRB 66 20 = 20/86=23.3
...
#This is a calculated measure based on mL and a calf was considered a refusal if they refused 25% or more milk at first milk feeding (500mLs)
table(clean$D0Fed2TxGrp, clean$refuseCALC)
#       0  1
#  CSB 24  6 = 5/30 = 16.6%
#  CST 20 10 = 9/30 = 30%
#  MCB 10 20 = 16/30 = 53.3%
#  MCT 16 13 = 11/29 = 37.9%
#  MRB 13 18 = 18/31 = 58%


CrossTable(clean$refuseCALC,  clean$CrossbredYN, fisher=TRUE) #no not associated
CrossTable(clean$refuseCALC,  clean$CalfSizeLgSm,  fisher=TRUE) #no
CrossTable(clean$refuseCALC,  clean$SexMF, fisher=TRUE) #no
CrossTable(clean$refuseCALC,  clean$D0Fed2TxGrp, fisher = TRUE) #yes
...
model <- glm(refuseCALC ~ D0Fed2TxGrp + CalfSizeLgSm, family = "binomial", data = clean)
...
OR_model <- exp(cbind(OR = coef(model), confint(model)))
print(OR_model)


#ORs for the Table in manuscript: 

# Setup factor and reference group
clean$D0Fed2TxGrp <- factor(clean$D0Fed2TxGrp) 
clean$D0Fed2TxGrp <- relevel(clean$D0Fed2TxGrp, ref = "MRB")
...
# Method 1: Quick look at ORs relative to MRB
round(exp(cbind(OR = coef(model), confint(model))), 3)

# Method 2: Clean EMMEANS output with adjusted p-values vs MRB
library(emmeans)
em_link <- emmeans(model, specs = "D0Fed2TxGrp", type = "link")
or_contrasts <- contrast(em_link, method = "trt.vs.ctrl", ref = 1)
summary(or_contrasts, type = "response")


#ALL comparisons: 
# 1. Get the estimated marginal means on the log-odds (link) scale
em_link <- emmeans(model, specs = "D0Fed2TxGrp")

# 2. Compute all pairwise comparisons and exponentiate to get Odds Ratios
pairwise_results <- contrast(em_link, method = "pairwise", type = "response")

# 3. View the results (includes ORs, Confidence Intervals, and Tukey-adjusted p-values)
summary(pairwise_results)
plot(pairwise_results, comparisons = TRUE)
```

### D1 Model using 50%

```{r}
#This is a calculated measure based on mL and a calf was considered a refusal if they refused 50% or more milk at first milk feeding (1000mLs)
table(hsd1$D0Fed2TxGrp, hsd1$refuse1L)
#       0  1
#  CSB 29  1 = 1/30 = 3.3%
#  CST 26  4 = 4/30 = 13%
#  MCB 23  8 = 8/31 = 25.8%
#  MCT 24  7 = 7/31 = 22.5%
#  MRB 19 11 = 11/30 = 36.6%


CrossTable(hsd1$refuse1L,  hsd1$CrossbredYN, fisher=TRUE) #no not associated
CrossTable(hsd1$refuse1L,  hsd1$CalfSizeLgSm,  fisher=TRUE) #no
CrossTable(hsd1$refuse1L,  hsd1$SexMF, fisher=TRUE) #yes
CrossTable(hsd1$refuse1L,  hsd1$D0Fed2TxGrp, fisher = TRUE) #yes
...
hsd1$D0Fed2TxGrp <- factor(hsd1$D0Fed2TxGrp) 
hsd1$D0Fed2TxGrp <- relevel(hsd1$D0Fed2TxGrp, ref = "MRB")
model <- glm(refuse1L ~ D0Fed2TxGrp + SexMF, family = "binomial", data = hsd1)
...
#       N  Y
#  CSB 19 12 = 12/31= 38.7
#  CST 19 12 = 12/31= 38.7
#  MCB 19 12 = 12/31= 38.7
#  MCT 23  8 = 8/31= 25.8
#  MRB 17 14 = 14/31= 45.1
...
CrossTable(clean$FarmDzYN,  clean$Replacement, fisher = TRUE) #no
...
## Graphs for Presentation (Scour, THS)
...
df <- data.frame(
  Group = c("MRB", "CSB", "CST", "MCB", "MCT"),
  event = c(15.3, 9.1, 22.0, 9.8, 11.5))
 
df$Group <- factor(df$Group, levels = c("MRB", "CSB", "CST", "MCB", "MCT"))

df |> 
  ggplot(aes(x = Group, y = event, fill = Group))+
  geom_bar(stat= "identity")+
  geom_text(aes(label = paste0(round(event, 1), "%")), 
            vjust = -0.5, size = 4) +
  ...
  theme_classic() +
  labs(x = NULL, 
       y = "Proportion of Observations FS > 1")+
...
df <- data.frame(
  Group = c("MRB", "CSB", "CST", "MCB", "MCT"),
  event = c(33.6, 16.2, 32.6, 23.2, 25.6))
...
df |> 
  ...
  theme_classic() +
  labs(x = NULL, 
       y = "Proportion of Observations THS > 3")+
    scale_y_continuous(limits = c(0, 100), breaks = seq(0, 100, by = 20))+
  theme(legend.position = "none", 
        text = element_text(family = "Arial", size = 14))
```
...
df <- data.frame(
  Group = c("MRB", "CSB", "CST", "MCB", "MCT"),
  event = c(58.0, 16.7, 30.9, 52.2, 39.8))
...
df <- data.frame(
  Group = c("MRB", "CSB", "CST", "MCB", "MCT"),
  event = c(36.2, 2.8, 11.0, 26.7, 20.5))
